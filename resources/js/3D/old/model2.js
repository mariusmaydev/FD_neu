...
export default class Model {
    ...
    static async init(instance, name = "lighter", loaderCount = 1, GoldFlag = true){
        ...
        if(FLAG_sceneLoaded < loaderCount){
            ...
            let gltfScene = SRCscene.clone();
            gltfScene.name = name;
            instance.scene.add(gltfScene);
            gltfScene.rotation.x = Math.PI / 2;

            // SPLINT.resources.models.lighter_glb.sceneL = gltfScene.clone();
    ...
    static async getThumbnail(scene, instance, src = instance.thumbnailSRC, name = "", color = 0xe8b000, isHidden = false){
        let double = false;
        let material = null;
            material = MATERIALS.Lighter.Engraving(instance, src, undefined, color, function(texture, mat = null){
                if(mat != null){
                    material = mat;
                }      
