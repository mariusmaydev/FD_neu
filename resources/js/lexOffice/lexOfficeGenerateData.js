class lexOfficeObjectGenerator {
    static async newInvoiceData(orderData, orderID){
        let sendingPrice = 0.00;
        ...
        let response = new Object();
            ...
            response.voucherDate    = lexOfficeHelper.createVoucherDate();
            ...
            for(let i = 0; i < shoppingCart.length; i++){
                let productData = await productHelper.getByName(shoppingCart[i].ProductName);
                response.lineItems[i] = new Object();
                //response.lineItems[0].id = "1";

                response.lineItems[i].type = "material ";
                response.lineItems[i].name = productData.viewName;
                response.lineItems[i].description = productData.description;
                ...
                response.lineItems[i].unitPrice.netAmount = parseFloat(productData.price);      //Netto
                response.lineItems[i].unitPrice.grossAmount = parseFloat(productData.price);                     //Netto
                FullPrice += parseFloat(productData.price);
                //response.lineItems[0].unitPrice.grossAmount = 4.99;    //Brutto
                response.lineItems[i].unitPrice.taxRatePercentage = 19;
            ...
            response.lineItems[shoppingCart.length].description = "Standart";
            ...
            response.lineItems[shoppingCart.length].unitPrice.netAmount = parseFloat(sendingPrice);      //Netto
            response.lineItems[shoppingCart.length].unitPrice.grossAmount = parseFloat(sendingPrice);      //Netto
            FullPrice += sendingPrice;
            response.lineItems[shoppingCart.length].unitPrice.taxRatePercentage = 19;
            ...
            // response.totalPrice.totalGrossAmount = 24.99;
            ...
            response.taxConditions.taxType = "gross";
            //response.taxConditions.taxTypeNote = null;

            response.paymentConditions = new Object();
            if(!SPLINT.API.Paypal.PAY_METHOD[orderData.paymentMethod].isEmpty()){
                response.paymentConditions.paymentTermLabel = "Bezahlt mit " + SPLINT.API.Paypal.PAY_METHOD[orderData.paymentMethod].name;
            } else {
  ...
  static async newDeliveryData(orderData, orderID){
        let sendingPrice = 0.00//3.75;
        let couponCode = orderData.couponCode;
        let shoppingCart = orderData.Items;
        let shippingAddress = orderData.Address.sending;
        let invoiceAddress = orderData.Address.invoice;
